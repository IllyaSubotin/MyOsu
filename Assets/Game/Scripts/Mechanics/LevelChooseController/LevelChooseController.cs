...
public class LevelChooseController : MonoBehaviour, ILevelChooseController, IPointerDownHandler, IDragHandler, IPointerUpHandler 
{
    [Header("Layout")]
    ...
    {
        if(levelItems.Count < _saveLoadManager.beatmapDatas.Count)
        {
            for(;_saveLoadManager.beatmapDatas.Count > levelItems.Count;)
            {
        ...
        {
            levelItems[i].nameText.text = _saveLoadManager.beatmapDatas[i].beatmapName;
        }
